...
function draw() {
    ...
    if (gamestate === "level1") {
        ...
        fill("green")
        textSize(40)
        text("TimeToSafeZone:-" + TimeToSafeZone, width / 4 - 200, height - 540)

        textFont("chiller")
        fill("green")
        textSize(40)
        text("BULLETS LEFT:-", width / 2, height - 540)
    }
    ...
    if (gamestate === "defeat") {
        ...
        fill("green")
        textFont("chiller")
        text("YOU LOSE", width / 2, lose.height - 400)
    }
...
function won() {
    background(0)
    var win = createSprite(width / 2, height / 2);
    win.addImage(wonimg);
    win.scale = 2;
    player.destroy();
    zombieGrp.destroyEach();
